...
public class MediaUtil {
     ...
     */
    private static final Time DEFAULT_TIME = new Time(10000);
    /**
     * 视频帧抽取的默认宽度值，单位：px
     */
    ...
    /**
     * FFmpeg程序执行路径 当前系统安装好ffmpeg程序并配置好相应的环境变量后，值为ffmpeg可执行程序文件在实际系统中的绝对路径 // /usr/bin/ffmpeg
     */
    private static String FFMPEG_PATH = "D:\\01_Software\\01_Video_Maker\\ffmpeg-2023-06-15\\bin\\ffmpeg.exe";// "/usr/bin/ffmpeg";
    ...
    public static boolean setFfmpegPath(String ffmpegPath) {
        ...
        log.info("--- 设置ffmpeg执行路径成功 --- 当前ffmpeg可执行程序路径为： " + ffmpegPath);
        return true;
    }
    ...
    public static String executeCommand(List<String> commands) {
        ...
        // 输出执行的命令信息
        String cmdStr2 = Arrays.toString(ffmpegCmds.toArray()).replace(",", "");
        log.info("--- 已执行的ffmpeg命令： ---" + cmdStr2);
        ...
        try {
            ...
            log.info("--- 已执行的ffmpeg命令： ---" + cmdStr + " 已执行完毕,执行结果： " + resultStr);
            return result;
        } catch (Exception e) {
    ...
    public static void cutVideoFrame(File videoFile, File fileOutPut, Time time, int width, int height) {
        ...
        if (!"GIF".equals(StringUtils.upperCase(format))) {
            ...
            String tempPath =
                path + File.separator + System.currentTimeMillis() + "_" + name.substring(0, name.indexOf("."));
    ...
    private static void cutVideoFrame(File videoFile, String path, Time time, int width, int height, int timeLength,
        ...
        if (time.getTime() + timeLength > info.getDuration()) {
            throw new RuntimeException(
                "开始截取视频帧的时间点不合法：" + time.toString() + "，因为截取时间点晚于视频的最后时间点");
        }
    ...
    public static void cutVideo(File videoFile, File outputFile, Time startTime, int timeLength) {
        long methodExecuteStartTime = System.currentTimeMillis();
        if (videoFile == null || !videoFile.exists()) {
...
//            command.add("copy");
        ...
        long period = endTime - methodExecuteStartTime;
    ...
    public static MusicMetaInfo getMusicMetaInfo(File musicFile) {
        ...
        Matcher musicStreamMatcher = musicStreamPattern.matcher(parseResult);
