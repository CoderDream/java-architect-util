...
//        System.out.println(dateStr);
        // 递归调用获取前一天，直到找不到为止
        JSoupWechatUtil.getWechatInfoList(url, wechatInfoList);
        // 获取当天
//        JSoupWechatUtil.crawlerCode(url, wechatInfoList);
     ...
     */
    public static String getPublishTime(String url) throws IOException {

        String targetLine = null, timeStampStr = null;

        //解析网页
        Document document = JSoupSwaggerUtil.getDocument(url); //Jsoup.parse(new URL(URL), 30000);
    ...
    public static void getWechatInfoList(String url, List<WechatInfo> wechatInfoList) {
        do {
            url = JSoupWechatUtil.crawlerCode(url, wechatInfoList);
        } while (StrUtil.isNotEmpty(url));
    }

    public static String crawlerCode(String url, List<WechatInfo> historyUrlList) {
        Document document = JSoupSwaggerUtil.getDocument(url);
        return getWechatInfoListByClassCode(document, YESTERDAY_CLASS, historyUrlList);
    }

    public static String getWechatInfoListByClassCode(Document document,
        String className, List<WechatInfo> historyUrlList) {
        String href = "";
        WechatInfo wechatInfo;
        /**
         * 限免日期
         */
        String freeDate = getPublishTime(document);
                ...
                for (int i = 5; i < size; i++) {
                    ...
                    if (childNode instanceof Element) {
                        ...
                        if (childElement.tag() != null
                            ...
                            wechatInfo.setFreeDate(freeDate);
                            appIndex++;
                            freeAppNo = freeDate + "_" + String.format("%02d", appIndex);
                            wechatInfo.setFreeAppNo(freeAppNo);
                            ...
                            wechatInfo.setTitle(title.trim());
                            wechatInfo.setPriceStr(priceStr);
                            wechatInfo.setCapabilityStr(capabilityStr.trim());
                            wechatInfo.setDescription(description.trim());
                            wechatInfo.setHref(href);
                            Elements imgElements = childElement.getElementsByClass(
                                "rich_pages"); //  wxw-img
                                ...
                                switch (imgSize) {
                                    case 1:
                                        ...
                                        if (imgElement.attributes() != null) {
                                            System.out.println(
                                                imgElement.attributes().get("data-src"));
                                    ...
                                    case 2:
                                        ...
                                        if (imgElement2.attributes() != null) {
                                            System.out.println(
                                                imgElement2.attributes().get("data-src"));
                                            snapshot = imgElement2.attributes().get("data-src");
                                        }
                                        Element imgElement3 = imgElements.get(1);
                                        if (imgElement3.attributes() != null) {
                                            System.out.println(
                                                imgElement3.attributes().get("data-src"));
                                            snapshot = imgElement3.attributes().get("data-src");
                                        }
                                        break;
                                }
                            }
                            wechatInfo.setSnapshot(snapshot);
//                            qrUrl = "";
                            if (StrUtil.isNotEmpty(snapshot) && StrUtil.isNotEmpty(freeDate)
                                && StrUtil.isNotEmpty(freeAppNo)) {
                                qrUrl = getAppUrl(wechatInfo.getSnapshot(), freeDate, freeAppNo);
                            }
                            wechatInfo.setQrUrl(qrUrl);
                            if (StrUtil.isNotEmpty(qrUrl)) {
                                appUrl = UrlUtils.tranAppUrl(qrUrl);
                                wechatInfo.setAppUrl(appUrl);
                            }
//                            System.out.println(wechatInfo);
                            historyUrlList.add(wechatInfo);
                        }
                    }
                }
            }
    ...
    public static String getAppUrl(String snapshot, String freeDate, String freeAppNo) {
        String path = freeDate;
        String picName = freeAppNo + ".png";
        String picQrName = freeAppNo + "_qr.png";
