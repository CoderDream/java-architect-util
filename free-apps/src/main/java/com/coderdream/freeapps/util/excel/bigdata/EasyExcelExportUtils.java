...
/**
 * https://www.cnblogs.com/dz-boss/p/15519431.html
...
public class EasyExcelExportUtils {
    ...
    public static void main(String[] args) {
        EasyExcelExportUtils.writeExcelByMulSheetAndWriteChange();
    }

    public static final String FILE_NAME = "D:\\test_" + System.currentTimeMillis() + ".xlsx";
    // 每个 sheet 写入的数据
    public static final int NUM_PER_SHEET = 999999;
     ...
     */
    public static void writeExcelByApi() {
        String fileName = FILE_NAME;
        log.error("导出excel名称={}", fileName);
        long startTime = System.currentTimeMillis();
        // 直接调用api
        List<AttrItemExportExcelDto> date = getDate();
        EasyExcel.write(fileName, AttrItemExportExcelDto.class).sheet().doWrite(date);
     ...
     */
    public static void writeExcelByMulSheet() {
        String fileName = FILE_NAME;
        log.error("导出excel名称={}", fileName);
        long startTime = System.currentTimeMillis();
        // 获取数据
        ...
        // 指定写入的文件
        ExcelWriter excelWriter = EasyExcel.write(fileName, AttrItemExportExcelDto.class).build();
        for (int i = 0; i < sheetNum; i++) {
            ...
            log.error("写入sheet={}，数据量{}-{}={}，耗时={}ms", sheetName, endNum, startNum, endNum - startNum,
                System.currentTimeMillis() - l);
        }
        // 最好放在 finally中
        excelWriter.finish();
        log.error("导出excel结束,总数据量={}，耗时={}ms", date.size(), System.currentTimeMillis() - startTime);
        Long period = System.currentTimeMillis() - startTime;
        System.out.println("导出excel结束,数据量=" + date.size() + "，耗时=" + period + "ms");
    }
     ...
     */
    public static void writeExcelByMulWrite() {
        String fileName = FILE_NAME;
        log.error("导出excel名称={}", fileName);
        long startTime = System.currentTimeMillis();
        // 获取数据
        List<AttrItemExportExcelDto> date = getDate();
        ExcelWriter excelWrite = EasyExcel.write(fileName, AttrItemExportExcelDto.class).build();
        ...
        for (int i = 0; i < times; i++) {
            ...
            log.error("写入数量{}-{}={}，耗时={}ms", endNum, startNum, endNum - startNum,
                System.currentTimeMillis() - l);
        }
        // 需要放入 finally 中
        if (excelWrite != null) {
     ...
     * Process finished with exit code 0
     */
    public static void writeExcelByMulSheetAndWriteChange() {
        String fileName = FILE_NAME;
        log.error("导出excel名称={}", fileName);
        long startTime = System.currentTimeMillis();
        // 获取数据
        ...
        for (int i = 0; i < sheetNum; i++) {
            ...
            for (int j = 0; j < writeNum; j++) {
                ...
                excelWriter.write(date.subList(startNum, endNum), writeSheet);
                log.error("写入sheet={}，数据量={}-{}={}，耗时={}", sheetName, endNum, startNum, endNum - startNum,
                    System.currentTimeMillis() - l);
            }
        }
        // 需要放入 finally 中
     ...
     * @return list 集合
     */
    public static List<AttrItemExportExcelDto> getDate() {
        log.error("开始生成数据");
        int DATA_COUNT = 399999;
        Date date = new Date();
        long startTime = System.currentTimeMillis();
        List<AttrItemExportExcelDto> list = Lists.newArrayList();
        for (int i = 0; i < DATA_COUNT; i++) {
            AttrItemExportExcelDto attrItemExportExcelDto = new AttrItemExportExcelDto();
            attrItemExportExcelDto.setAttrItemName("地理信息");
            attrItemExportExcelDto.setAttrItemCode("00");
            attrItemExportExcelDto.setAttrItemFullCode("0A0001B010X00");
            attrItemExportExcelDto.setAttrItemLabel("gis");
            attrItemExportExcelDto.setObjectTypeName("行政区_市");
            attrItemExportExcelDto.setObjectName("杭州市");
            attrItemExportExcelDto.setObjectCode("0001");
            attrItemExportExcelDto.setStructureTypeName("基础数据");
            attrItemExportExcelDto.setAttrTypeName("标识信息");
            attrItemExportExcelDto.setAttrTypeCode("01");
            attrItemExportExcelDto.setAttrName("地理信息");
            attrItemExportExcelDto.setAttrCode("0X");
            attrItemExportExcelDto.setDataType("面状地理数据");
            attrItemExportExcelDto.setHashRateLevel("L0");
            attrItemExportExcelDto.setDataSource("系统来源");
            attrItemExportExcelDto.setRemark("备注");

            list.add(attrItemExportExcelDto);
        }
