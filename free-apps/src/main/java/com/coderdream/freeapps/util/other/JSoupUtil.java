...
//        System.out.println(app);
    ...
    public static Integer RETRY_TIMES = 5;
    ...
    public static AppEntity crawlerApp(String appId, Integer usFlag) {
        ...
        try {
            ...
            if (usFlag == null || usFlag == 0) {
                document = JSoupUtil.getDocument(urlCn, RETRY_TIMES);
                if (document == null) {
                    document = JSoupUtil.getDocument(urlUs, RETRY_TIMES);
...
//        app.setPriceStr(appInfo.getPrice()); 不能用这里的价格信息，因为不包含是否应用内购
            ...
            if (StrUtil.isNotEmpty(description) && (description.contains("简介 ") || description.contains(
                "Description "))) {
            ...
            if (CollectionUtils.isEmpty(screenshotsList)) {
                ...
                document = JSoupUtil.getDocument(urlRaw, RETRY_TIMES);
...
//    }
    ...
    public static Document getDocument(String url, Integer retryTimes) {
        Connection conn = Jsoup.connect(url);
//        conn.userAgent("custom user agent");
        Document document = null;
        try {
...
//            return null;
            ...
            if (retryTimes < 0) {
                ...
                return getDocument(url, --retryTimes);
            }
        }
        return document;
    }
