...
@Slf4j
public class JSoupZpUtil {

    private static final Logger logger = org.slf4j.LoggerFactory.getLogger(JSoupZpUtil.class);
    /**
...
//        https://apps.apple.com/cn/app/id441878713
        ...
        AppEntity app = JSoupZpUtil.crawlerApp(appId, null);
    ...
    public static AppEntity crawlerApp(String appId, Integer usFlag) {
        ...
        try {
            ...
            if (usFlag == null || usFlag == 0) {
                document = JSoupZpUtil.getDocument(urlCn);
                if (document == null) {
                    document = JSoupZpUtil.getDocument(urlUs);
...
//        app.setPriceStr(appInfo.getPrice()); 不能用这里的价格信息，因为不包含是否应用内购
            ...
            if (CollectionUtils.isEmpty(screenshotsList)) {
                ...
                document = JSoupZpUtil.getDocument(urlRaw);
