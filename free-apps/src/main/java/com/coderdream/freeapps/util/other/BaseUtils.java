...
//        List<AppBrief> appBriefList = BaseUtils.genBrief();
        List<AppBrief> appBriefList = BaseUtils.genBriefByWechat();
...
//        BaseUtils.splitAppBriefList(3, appBriefList);
    ...
    public static List<AppBrief> genBrief() {
        ...
        List<String> rawStrList = TxtUtil.readTxtFileToList(txtFileName);
...
//            fileName = String.format("%02d", z) + "_" + appId;
    ...
    public static List<AppBrief> genBriefByWechat() {
        List<AppBrief> appBriefList = new ArrayList<>();
        AppBrief appBrief;
        String monthStr = new SimpleDateFormat("yyyyMM").format(new Date());
        String dateStr = new SimpleDateFormat("yyyyMMdd").format(new Date());
        String path = BaseUtils.getPath();
        String txtFileName = File.separator + path + File.separator + "wechat" + ".txt";
...
//        String osType;
        ...
        for (Map.Entry<Integer, Integer> entry : freeIndexMap.entrySet()) {
            appBrief = new AppBrief();
            Integer freeIndex = entry.getValue();
                name = rawStrList.get(freeIndex-2);
            System.out.println(name);
//            System.out.println("name: " + rawStrList.get(freeIndex-2));
//            System.out.println("price: " + rawStrList.get(freeIndex-1));
//            System.out.println("description: " + rawStrList.get(freeIndex));

//            tempStr = rawStrList.get(lastIndex);
//            if("  ▼长按扫码下载▼ ".equals(tempStr.trim()) || "图片".equals(tempStr.trim())) {
//                continue;
//            }
//
//            if ("今日苹果iOS 限免软件".equals(tempStr) && osType.isEmpty()) {
...
//            System.out.println(lastIndex + "_" + freeIndex);
//            lastIndex++;
            String priceStr = rawStrList.get(freeIndex-1);
            ...
            priceStr = priceStr.replaceAll("免费", "");
            priceStr = priceStr.replaceAll("iPhone", "");
            priceStr = priceStr.replaceAll("【", "");
            priceStr = priceStr.replaceAll("】", "");
            priceStr = priceStr.replaceAll("➜", "");
            priceStr = priceStr.replaceAll("·", "");
            priceStr = priceStr.replaceAll(" ", "");
            priceStr = priceStr.replaceAll("iPad", "");
//            ¥25➱0
            priceStr = "¥"+ priceStr +"➱0";

            System.out.println(priceStr);
            String description = rawStrList.get(freeIndex);
            description = description.replaceAll("简介：", "");
