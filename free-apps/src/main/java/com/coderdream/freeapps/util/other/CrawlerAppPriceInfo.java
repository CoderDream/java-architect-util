...
@Slf4j
public class CrawlerAppPriceInfo {

    private static final Logger logger = org.slf4j.LoggerFactory.getLogger(CrawlerAppPriceInfo.class);
    /**
...
//        https://apps.apple.com/cn/app/id441878713
        ...
        appId = "id1071186450";
        appId = "id1454412797"; // delete_flag 为空
        ...
        AppEntity app = CrawlerAppPriceInfo.crawlerApp(appId, null);
    ...
    public static AppEntity crawlerApp(String appId, Integer usFlag) {
        ...
        try {
            ...
            if (usFlag == null || usFlag == 0) {
                document = CrawlerAppPriceInfo.getDocument(urlCn);
                if (document == null) {
                    document = CrawlerAppPriceInfo.getDocument(urlUs);
                    if (document == null) {
                        logger.error("urlCn：" + appId + " 已下架！");
                        app.setRemark(appId + " 已下架！");
                        app.setDeleteFlag(1); // 设为删除
                        return app;
                    }
                    usFlag = 1;
                    app.setUsFlag(1); // 美区
...
//        app.setPriceStr(appInfo.getPrice()); 不能用这里的价格信息，因为不包含是否应用内购
            ...
            if (StrUtil.isNotEmpty(priceStrAppInfo)) {
                ...
                try {
                    ...
                    if (StrUtil.isNotEmpty(realPrice) && !realPrice.equals("Free") && !realPrice.equals("免费")) {
                        if (usFlag == 1) {
                            app.setPriceUs(BigDecimal.valueOf(Double.parseDouble(realPrice)));
                        } else {
            ...
            if (StrUtil.isNotEmpty(ratingStr) && ratingStr.split(CdConstants.MIDDLE_POINT).length == 2) {
                ...
                if (StrUtil.isNotEmpty(ratingTemp)) {
                    if (ratingTemp.lastIndexOf("K") != -1) {
                        ...
                        app.setRating(BigDecimal.valueOf(Double.parseDouble(ratingTemp) * 1000));
                    } else if (ratingTemp.lastIndexOf("万") != -1) {
                        ratingTemp = ratingTemp.replaceAll("万", "");
                        app.setRating(BigDecimal.valueOf(Double.parseDouble(ratingTemp) * 10000));
                    } else {
                        app.setRating(BigDecimal.valueOf(Double.parseDouble(ratingTemp)));
                    }
                }
                if (StrUtil.isNotEmpty(rateAmountTemp)) {
                    if (rateAmountTemp.lastIndexOf("万") != -1) {
                        rateAmountTemp = rateAmountTemp.replaceAll("万", "");
                        double d = Double.parseDouble(rateAmountTemp);
                        d *= 10000;
                        app.setRateAmount((int) d);
            ...
            if (StrUtil.isNotEmpty(description) && (description.contains("简介 ") || description.contains("Description "))) {
                description = description.replace("简介 ", "");
            ...
            if (CollectionUtils.isEmpty(screenshotsList)) {
                ...
                document = CrawlerAppPriceInfo.getDocument(urlRaw);
