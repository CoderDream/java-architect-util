...
public class MyBatisPlusWrapperTest {
    ...
    public void test4() {
        ...
        QueryWrapper<User2> queryWrapper = new QueryWrapper<>(); //设置条件 直接写 直接写
        queryWrapper.gt("age", 19) //gt greater than 大于
                .like("user_name", "a") // 因为 是并且关系 因此 直接写因为是并且关系因此直接写
                .or()// or 或
                .isNull("email");
        User2 user = new User2();
        user.setName("小a");  // 修改的内容
        user.setEmail("xiaoA@163.com"); // 修改的内容
        int result = user2Mapper.update(user, queryWrapper); //第一个参数是实体内容，第二个参数是查询条件
        System.out.println("result：" + result);
    }
    ...
    public void test42() {
        ...
        QueryWrapper<User2> queryWrapper = new QueryWrapper<>(); //设置条件
        queryWrapper.like("user_name", "a")
    ...
    public void test10() {
        ...
        queryWrapper.like(StringUtils.isNotBlank(username), User2::getName, username)
    ...
    public void test11() {
        ...
        updateWrapper.like(User2::getName, "a")
                .and(i -> i.gt(User2::getAge, 19).or().isNull(User2::getEmail));
        updateWrapper.set(User2::getName, "LambdaUpdateWrapper修改").set(User2::getEmail, "abc改@163.com");
        int result = user2Mapper.update(null, updateWrapper);
        System.out.println("result：" + result);
    }
}
