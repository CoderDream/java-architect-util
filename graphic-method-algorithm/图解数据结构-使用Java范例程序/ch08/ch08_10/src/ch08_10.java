...
{	
	public static void main (String args[])throws Exception 
	{
		...
		if(!fp.exists())		//文件是否打开成功
			System.out.print("打开数据文件失败\n");
		else if(!fp1.exists())
			System.out.print("打开分割文件 1 失败\n");	
		else if(!fp2.exists())		
			System.out.print("打开分割文件 2 失败\n");
		else if(!fpa.exists())		
			System.out.print("打开合并文件失败\n");
		else
		{  
			System.out.print("正在分割文件......\n");
	        me(fp,fp1,fp2,fpa);
			System.out.print("正在对数据进行排序......\n");
			System.out.print("数据处理完成！！！\n");
		}

		System.out.print("原始文件datafile.txt的数据内容为：\n");
		showdata(fp);
		System.out.print("\n分割文件sort1.txt的数据内容为：\n");
		showdata(fp1);
		System.out.print("\n分割文件sort2.txt的数据内容为：\n");
		showdata(fp2);
		System.out.print("\n排序后sortdata.txt的数据内容为：\n");
		showdata(fpa);
	}
	...
	{  
		char str1,str2;
		int n1,n2;	//声明变量n1，n2，用于暂存数据文件data1及data2内的元素值
		...
		{  
			...
			{  
				str1=(char)n1;
				pfile.write(str1);	//如果n1比较小，则把n1存储到fp中
				n1=pfile1.read();	//接着读下一项 n1 的数据
			}
			else
			{  
				str2=(char)n2;
				pfile.write(str2);	//如果n1比较小，则把n1存储到fp中
	            n2=pfile2.read();	//接着读下一项 n2的数据
			}
		}
		if(n2!=-1)	//如果其中一个数据文件已读取完毕，经判断后
		{ 		    //把另一个数据文件内的数据全部存储到fp中
			while (true)
			{  
				if(n2==-1)
					 break;
